...
function renderProducts(){
  els.grid.innerHTML = state.filtered.map(p=>{
    const catAttr = `data-cat="${p.categoria}"`;
    const isRetina = p.img.endsWith('.webp') ? `${p.img} 1x, ${p.img.replace('.webp','@2x.webp')} 2x` : '';
    const ratioClass = p.categoria === 'macetas' ? 'ratio-1x1' : 'ratio-4x3';
    return `
    <div class="col-12 col-sm-6 col-lg-4 col-xl-3">
      <div class="product-card glass h-100 d-flex flex-column" ${catAttr}>
        <div class="ratio ${ratioClass} mb-2 product-thumb">
          <img src="${p.img}" ${isRetina?`srcset="${isRetina}"`:''} alt="${p.nombre}" class="w-100 h-100">
        </div>
